...
public class SessionListAdapter extends FirebaseListAdapter<Session> {
    ...
    public SessionListAdapter(Query ref, AppCompatActivity activity, int layout, int listType,boolean searchingFlag) {
        super(ref, Session.class, layout, activity, searchingFlag);
        mListType = listType;
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        ...
        if(holder instanceof ViewHolderH){//Header View
            ...
            iholder.mItem = s;
            final String lastMessage = s.getLastMessage();
            ...
            int mNumChildren = s.getFollowers();
            if (mNumChildren > 0) {
                iholder.mFollowersView.setVisibility(View.VISIBLE);
                iholder.mFollowersView.setText(mNumChildren + " " + mCtx.getString(R.string.single_follower_string)+(mNumChildren == 1?"":"s"));
                iholder.mFollowersView.invalidate();
            }
            ...
            loadBitmap(s.getPicture(), iholder.mIconView,iholder.mProgressImgView,false);
            if(mListType==LIST)
